      ...
      ROLES.forEach(role => {
        ...
        role.connectsTo.forEach(toId => {
          ...
          if (role.level === toRole.level) {
            ...
            cross = (toRole.level - role.level) > 1   // skip-level → lighter
            d = `M ${cx(fromEl, cr.left)} ${bot(fromEl, cr.top)} L ${cx(toEl, cr.left)} ${top(toEl, cr.top)}`
          } else {
            // ── Cross-column connector ─────────────────────────────────────────
            // Route through empty inter-row space and arrive at the SIDE MIDPOINT
            // of the target card (left edge if coming from the left, right edge if
            // coming from the right). This means multiple connections to the same
            // card spread across its height rather than all piling at the top.
            cross = true
            const LANE  = 5   // px clearance in column gap
            const tgtCY = cy(toEl, cr.top)   // vertical centre of target card

            if (toRole.level > role.level) {
              // ── Downward ────────────────────────────────────────────────────
              const srcRow = fromEl.closest('.org-row') as HTMLElement | null

              // busY: below every card in the source row
              let busY = bot(fromEl, cr.top) + 4
              if (srcRow) {
                let maxBot = srcRow.getBoundingClientRect().bottom - cr.top
                srcRow.querySelectorAll<HTMLElement>('.org-card').forEach(el => {
                  maxBot = Math.max(maxBot, el.getBoundingClientRect().bottom - cr.top)
                })
                busY = maxBot + 4
              }

              const goingRight = role.track < toRole.track
              // Vertical lane in the column gap beside the target
              const colGapX = goingRight
                ? lft(toEl, cr.left) - LANE
                : rgt(toEl, cr.left) + LANE
              // Arrow arrives at the side edge at the card's vertical centre
              const entryX = goingRight ? lft(toEl, cr.left) : rgt(toEl, cr.left)

              d = [
                `M ${cx(fromEl, cr.left)} ${bot(fromEl, cr.top)}`,  // exit src bottom-center
                `L ${cx(fromEl, cr.left)} ${busY}`,                  // drop to row bus
                `L ${colGapX} ${busY}`,                              // horiz in inter-row gap
                `L ${colGapX} ${tgtCY}`,                             // vert in column gap to card mid
                `L ${entryX}  ${tgtCY}`,                             // enter card side at midpoint
              ].join(' ')
            } else {
              // ── Upward or sideways (rare) ────────────────────────────────────
              const srcY  = cy(fromEl, cr.top)
              const tgtCX = cx(toEl,   cr.left)
              const tgtTopY = top(toEl, cr.top)
              if (role.track < toRole.track) {
                d = [`M ${rgt(fromEl, cr.left)} ${srcY}`, `L ${tgtCX} ${srcY}`, `L ${tgtCX} ${tgtTopY}`].join(' ')
              } else {
                d = [`M ${lft(fromEl, cr.left)} ${srcY}`, `L ${tgtCX} ${srcY}`, `L ${tgtCX} ${tgtTopY}`].join(' ')
              }
            }
          }
      ...
      <div className="org-scroll-area">
        ...
        <div className="org-grid" ref={containerRef}>
          <svg ref={svgRef} className="org-svg" aria-hidden="true">
            <defs>
              <marker id="arr-main"  markerWidth="5" markerHeight="5" refX="4" refY="2.5" orient="auto">
                <path d="M 0 0 L 4 2.5 L 0 5 Z" fill="#475569" />
              </marker>
              <marker id="arr-cross" markerWidth="4" markerHeight="4" refX="3.5" refY="2" orient="auto">
                <path d="M 0 0 L 3.5 2 L 0 4 Z" fill="#94a3b8" />
              </marker>
            </defs>
            {paths.map(p => (
              <path
                ...
                strokeWidth={p.cross ? 1.5 : 2}
                fill="none"
                markerEnd={p.cross ? 'url(#arr-cross)' : 'url(#arr-main)'}
              />
            ))}
          </svg>
